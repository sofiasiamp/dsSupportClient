...
init.testing.datasets <- function()
{
  ...
  if (ds.test_env$secure_login_details)
  {
    ...
    local.values.1.name        <- read.csv(here::here("tests/testthat/testsets", "Study1.csv"), sep = ";", dec = ",")
    ds.test_env$local.values.1 <- local.values.1.name
    #ds.test_env$local.values.1 <- eval(as.symbol(local.values.1.name))
    local.values.2.name        <- read.csv(here::here("tests/testthat/testsets", "Study2.csv"), sep = ";", dec = ",")
    ds.test_env$local.values.2 <- local.values.2.name
    local.values.3.name        <- read.csv(here::here("tests/testthat/testsets", "Study3.csv"), sep = ";", dec = ",")
    ds.test_env$local.values.3 <- local.values.3.name
    local.values.4.name        <- read.csv(here::here("tests/testthat/testsets", "Study4.csv"), sep = ";", dec = ",")
    ds.test_env$local.values.4 <- local.values.4.name
    ds.test_env$local.values   <- rbind(ds.test_env$local.values.1,ds.test_env$local.values.2,ds.test_env$local.values.3, ds.test_env$local.values.4)
    if (ds.test_env$driver == "OpalDriver") {
      builder <- DSI::newDSLoginBuilder(.silent = TRUE)
      builder$append(server = "study1", url = ds.test_env$ip_address_1, user = ds.test_env$user_1, password = ds.test_env$password_1, table = "TESTING.DATA1")
      builder$append(server = "study2", url = ds.test_env$ip_address_2, user = ds.test_env$user_2, password = ds.test_env$password_2, table = "TESTING.DATA2")
      builder$append(server = "study3", url = ds.test_env$ip_address_3, user = ds.test_env$user_3, password = ds.test_env$password_3, table = "TESTING.DATA3")
      builder$append(server = "study4", url = ds.test_env$ip_address_4, user = ds.test_env$user_4, password = ds.test_env$password_4, table = "TESTING.DATA4")
...
{
  #print(ds.test_env$login.data)
  #ds.test_env$login.data$driver <- "DSLiteDriver"
  #ds.test_env$driver <- "DSLiteDriver"
  #ds.test_env$login.data$url <- "ds.test_env"
  ds.test_env$login.data$table <- c("local.values.1", "local.values.2", "local.values.3", "local.values.4")
  #ds.test_env$login.data$table <- c("local.values.1", "local.values.2", "local.values.3", "local.values.4")
  #ds.test_env$login.data$table <- c("local.values.1", "local.values.2", "local.values.3", "local.values.4")

  ds.test_env$connections <- DSI::datashield.login(logins=ds.test_env$login.data, assign=TRUE, symbol = "D")
}
